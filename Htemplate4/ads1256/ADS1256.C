...
float ADS1256GetCap(void) //读取数据
{
	double data=0;
  data=ADS1256ReadData(ADS1256_MUXP_AIN0, ADS1256_MUXN_AINCOM);
	float rounded_num = data = data*calib1256*1000;
  return rounded_num;	
}
//-----------------------------------------------------------------//
//	函 数 名：ADS1256ReadData
//	形    参: AINP--正输入  AINN负输入 //负输入为AINN-CIOM则为单端输入，ANNX为差分  GAIN为设置的增益
//	返 回 值: 电压数据*calib1256*1000
...
double ADS1256GetVolt(uint8_t AINP_,uint8_t AINN_,int GAIN) //读取数据
{
  ...
  data=ADS1256ReadData(AINP_, AINN_);
	...
	float rounded_num = roundf(data * 100) / 100.0f;
